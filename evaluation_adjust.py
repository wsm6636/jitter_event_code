...
def output_adjust_G2023_MRT(num_repeats, random_seed, timestamp, results, false_results, num_chains):
    folder_path = "adjust"
    os.makedirs(folder_path, exist_ok=True)

    results_csv = os.path.join(folder_path, f"data_MRT_{num_repeats}_{random_seed}_{timestamp}.csv" )
    R_plot_name = os.path.join(folder_path, f"R_MRT_{num_repeats}_{random_seed}_{timestamp}.png")
    runtime_name = os.path.join(folder_path, f"Runtime_MRT_{num_repeats}_{random_seed}_{timestamp}.png")
    # save results to csv
    with open(results_csv, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["seeds","num_tasks", "final_e2e_max", "mrt", "R", "exceed", "false_percentage", "adjust", "inserted","run_time_our","run_time_G"])
        for num_tasks in num_chains:
            false_percentage = false_results[num_tasks]
            for (final_e2e_max, mrt, r, tasks, seed, exceed,adjust,inserted,run_time_our,run_time_G) in results[num_tasks]:
                writer.writerow([seed,num_tasks, final_e2e_max, mrt, r, exceed, false_percentage,adjust,inserted,run_time_our,run_time_G])

    print(f"All results saved to {results_csv}")

    # plot_histogram_MRT(results_csv, R_plot_name)
    # print(f"Plots generated and saved to {R_plot_name}")
    # plot_runtime(results_csv,runtime_name)
    # print(f"Plots generated and saved to {runtime_name}")
    return results_csv


def output_adjust_G2023_LET(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters):
    folder_path = "adjust"
    os.makedirs(folder_path, exist_ok=True)

    results_csv = os.path.join(folder_path, f"data_LET_{num_repeats}_{random_seed}_{timestamp}.csv" )
    R_plot_name = os.path.join(folder_path, f"R_LET_{num_repeats}_{random_seed}_{timestamp}.png")
    runtime_name = os.path.join(folder_path, f"Runtime_LET_{num_repeats}_{random_seed}_{timestamp}.png")
    # save results to csv
    with open(results_csv, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["seeds","num_tasks", "final_e2e_max", "letG2023", "R", "exceed", "false_percentage", "adjust", "inserted","run_time_G","run_time_our"])
        for num_tasks in num_chains:
            for per_jitter in jitters:
                false_percentage = false_results[num_tasks][per_jitter]
                for (final_e2e_max, letG2023, r, tasks, seed, exceed,adjust,inserted,run_time_G,run_time_our) in results[num_tasks][per_jitter]:
                    writer.writerow([seed,num_tasks, final_e2e_max, letG2023, r, exceed, false_percentage,adjust,inserted,run_time_G,run_time_our])

    print(f"All results saved to {results_csv}")

    # plot_histogram_LET(results_csv, R_plot_name)
    # print(f"Plots generated and saved to {R_plot_name}")
    # plot_runtime(results_csv,runtime_name)
    # print(f"Plots generated and saved to {runtime_name}")
...
def output_adjust(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters):

    # folder_name = f"{num_repeats}_{random_seed}_{timestamp}"
    # folder_path = os.path.join("adjust", folder_name)
    folder_path = "adjust"
    os.makedirs(folder_path, exist_ok=True)
    ...
    log_txt = os.path.join(f"log/adjust_log_{num_repeats}_{random_seed}_{timestamp}.txt")
...
def run_adjust(jitters, num_chains, num_repeats, random_seed, periods):
    TOLERANCE = 1e-9
    ...
    for i in range(num_repeats):            # loop on number of repetitions
        ...
        for num_tasks in num_chains:        # on number of tasks in a chain
            ...
            for per_jitter in jitters:      # on relative (to period) magnitude of jitter
                ...
                final_e2e_max, max_reaction_time,  final_r, final_w, tasks, adjust, inserted = run_analysis_adjust(num_tasks, selected_periods,selected_read_offsets,selected_write_offsets, per_jitter)
