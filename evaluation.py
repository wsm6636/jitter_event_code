...
def output_G2023_MRT(num_repeats, random_seed, timestamp, results, false_results, num_chains):
    folder_path = "rtssresult"
    ...
    with open(results_csv, mode='w', newline='') as file:
        ...
        writer.writerow(["seeds","num_tasks",  "final_e2e_max", "mrt", "R", "exceed", "false_percentage","run_time_our","run_time_G"])
        for num_tasks in num_chains:
            false_percentage = false_results[num_tasks]
            for (final_e2e_max, mrt, r, tasks, seed, exceed,run_time_our,run_time_G) in results[num_tasks]:
                writer.writerow([seed,num_tasks, final_e2e_max, mrt, r, exceed, false_percentage,run_time_our,run_time_G])
...
def output_G2023_LET(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters):
    folder_path = "rtssresult"
    os.makedirs(folder_path, exist_ok=True)
    ...
    with open(results_csv, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["seeds","num_tasks",  "final_e2e_max", "letG2023", "R", "exceed", "false_percentage","run_time_our","run_time_G"])
        for num_tasks in num_chains:
            for per_jitter in jitters:
                false_percentage = false_results[num_tasks][per_jitter]
                for (final_e2e_max, letG2023, r, tasks, seed, exceed,run_time_our,run_time_G) in results[num_tasks][per_jitter]:
                    writer.writerow([seed,num_tasks, final_e2e_max, letG2023, r, exceed, false_percentage,run_time_our,run_time_G])
...
def output_our(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters):
...
if __name__ == "__main__":
    ...
    run(jitters, num_chains, num_repeats, random_seed, periods)
