...
from evaluation import output_results_zero
...
def generate_periods_and_offsets_zero( periods):

    selected_read_offsets = [0 for period in periods]
    selected_write_offsets = [read_offset + period for read_offset, period in zip(selected_read_offsets, periods)]
    print(f"selected_periods: {periods}, selected_read_offsets: {selected_read_offsets}, selected_write_offsets: {selected_write_offsets}")
...
def compare_zero(jitters, num_chains, num_repeats, random_seed, periods):
    TOLERANCE = 1e-9
    # preparing list for storing result
    results = {num_tasks: {per_jitter: [] for per_jitter in jitters} for num_tasks in num_chains}
    final = {num_tasks: {per_jitter: [] for per_jitter in jitters} for num_tasks in num_chains}
    ...
    for i in range(num_repeats):
        ...
        for num_tasks in num_chains:
            selected_periods, selected_read_offsets, selected_write_offsets = generate_periods_and_offsets(num_tasks, periods)
            selected_read_offsets_zero, selected_write_offsets_zero = generate_periods_and_offsets_zero(selected_periods)
            for per_jitter in jitters:
                print(f"=========For evaluation========= num_tasks {num_tasks} per_jitter {per_jitter} Repeat {i} random_seed {random_seed} ==================")
                final_e2e_max, max_reaction_time,  final_r, final_w, tasks = run_analysis(num_tasks, selected_periods,selected_read_offsets,selected_write_offsets, per_jitter)
                if final_e2e_max != 0:
                ...
                print(f"=========For evaluation zero========= num_tasks {num_tasks} per_jitter {per_jitter} Repeat {i} random_seed {random_seed} ==================")
                final_e2e_max_zero, max_reaction_time_zero,  final_r_zero, final_w_zero, tasks_zero = run_analysis(num_tasks, selected_periods,selected_read_offsets_zero,selected_write_offsets_zero, per_jitter)
                # value of rate "= max_reaction_time / final_e2e_max"
                if final_e2e_max_zero != 0:
                    r_zero = max_reaction_time_zero / final_e2e_max_zero
                    if r_zero > 1 + TOLERANCE:  # if rate is larger than 1, then algorithm failed
                        exceed_zero = "exceed"
                    else:
                        exceed_zero = "safe"
                else:
                    r_zero = None
                    exceed_zero = None
                    false_results_zero[num_tasks][per_jitter] += 1  # algorithm failed

                results_zero[num_tasks][per_jitter].append((final_e2e_max_zero, max_reaction_time_zero, r_zero, tasks_zero, random_seed, exceed_zero))
    ...
    return results, false_results, final, results_zero, false_results_zero, final_zero
...
def compare_plots(csv_files, num_repeats, random_seed, timestamp):
    ...
    folder_path = os.path.join("compare/zero", folder_name)
...
def old_results():
    ...
    results, false_results, final, results_zero, false_results_zero, final_zero = compare_zero(jitters, num_chains, num_repeats, random_seed, periods)
    ...
    csv_file_zero, _, _, _ = output_results_zero(num_repeats, random_seed, timestamp, results_zero, false_results_zero, num_chains, jitters)
    ...
    csv_files = [csv_file, csv_file_zero]
...
def main():
    ...
    print(f"next experiment random_seed: {random_seed + num_repeats}")
