    ...
    with open(results_csv, mode='w', newline='') as file:
        ...
        for num_tasks in num_chains:
            for per_jitter in jitters:
                ...
                for (final_e2e_max_read, r_read, exceed_read, final_e2e_max_write, r_write, exceed_write, max_reaction_time, tasks, seed) in run_results[num_tasks][per_jitter]:
...
def run_LET(jitters, num_chains, num_repeats, random_seed, periods):
    ...
    for i in range(num_repeats):            # loop on number of repetitions
        ...
        for num_tasks in num_chains:        # on number of tasks in a chain
            ...
            for per_jitter in jitters:      # on relative (to period) magnitude of jitter
                ...
                results[num_tasks][per_jitter].append((final_e2e_max_read, r_read, exceed_read, final_e2e_max_write, r_write, exceed_write, max_reaction_time, tasks, random_seed))
    ...
    return false_results_write, false_results_read, results
