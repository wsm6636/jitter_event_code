...
def output_results(num_repeats, random_seed, timestamp, results, false_results, num_chains, jitters, chain_types):
    ...
    type_plot_name = os.path.join(folder_path, f"type_{num_repeats}_{random_seed}_{timestamp}.png")
    ...
    type_percent_order(type_plot_name, results_csv)
    ...
    for i in range(num_repeats):  # Loop on number of repetitions
        ...
        for num_tasks_index, num_tasks in enumerate(num_chains):  # Loop on number of tasks in a chain
            ...
            for per_jitter_index, per_jitter in enumerate(jitters):  # Loop on relative (to period) magnitude of jitter
                ...
                chain_results, max_reaction_time, tasks = run_analysis(num_tasks, selected_periods, selected_read_offsets, selected_write_offsets, per_jitter, chain_types)
...
if __name__ == "__main__":
    ...
    num_repeats = 300  # number of repetitions: if 10 takes about 20 minutes on Shumo's laptop
    ...
    # chain_types = ['asc', 'desc']
    # chain_types = ['max_period', 'min_period']

    run_results, false_results = run(jitters, num_chains, num_repeats, random_seed, periods, chain_types)
    
    output_results(num_repeats, random_seed, timestamp, run_results, false_results, num_chains, jitters, chain_types)
