...
def main():
    ...
    percent_plot_name = f"F16copy/percent_{num_repeats}_{random_seed}_{timestamp}.png"
    R_plot_name = f"F16copy/R_{num_repeats}_{random_seed}_{timestamp}.png"
    results_csv = f"F16copy/data_{num_repeats}_{random_seed}_{timestamp}.csv" 
    ...
    for i in range(num_repeats):            # loop on number of repetitions
        ...
        for num_tasks in num_chains:        # on number of tasks in a chain
            ...
            for per_jitter in jitters:      # on relative (to period) magnitude of jitter
                ...
                final_e2e_max, max_reaction_time,  final_r, final_w = run_analysis(num_tasks, selected_periods,selected_read_offsets,selected_write_offsets, per_jitter)
                ...
                results[num_tasks][per_jitter].append((final_e2e_max, max_reaction_time,r))
    ...
    with open(results_csv, mode='w', newline='') as file:
        ...
        for num_tasks in num_chains:
            for per_jitter in jitters:
                ...
                for (final_e2e_max, max_reaction_time, r) in results[num_tasks][per_jitter]:
                    writer.writerow([num_tasks, per_jitter, final_e2e_max, max_reaction_time, r, false_percentage])
