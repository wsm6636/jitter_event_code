    ...
    try:
        if argsg == "waters":
            ...
            while len(task_sets_waters) < argsr:
                ...
                # print(f"task sets created: {task_sets_gen[0]}", end="\r")
...
def schedule_taskset_as_list(ce_ts):
    ...
    print(f"Schedule result: {dict_res}.")
    list_res = []
    for task in ce_ts[1]:
...
def our_mrt_mRda(lst, bcet):
    ...
    print(f"BCET {rat_ts} {rat_sched}.")
    print(f" WCET {ts} {sched}")
...
def our_mrt_mRda_lst(lst_ce_ts, bcet_lst, wcet=1.0):
    ...
    for et in schedules_todo:
        ...
        ts_lst[et] = ts_et
        # print(f"Schedule for {et=}: {sched_et}.")
